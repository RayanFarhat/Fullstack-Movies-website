import { type ReqGoPro, type ResGoPro } from '../FrontBackSharedTypes';
import { getByUsernameDB, updateDBispro } from '../database';
import { sessionManager } from '../sessionManager';

export async function getPro(req: ReqGoPro): Promise<ResGoPro> {
    const username = await sessionManager.get(req.sid);
    const userdata = await getByUsernameDB(username);

    if (userdata.email !== '') {
        await updateDBispro(userdata.username, true);
        return {
            isAccepted: true,
            proContent: `[
                {
